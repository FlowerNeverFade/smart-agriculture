...
AI_MODE=openai-compatible  # openai-compatible | rules-only | mock | maxkb（外部不可用时明确降级）
LLM_ENABLE_THINKING=false  # 对话只生成最终答复，不启用思考模式
LLM_REASONING_EFFORT=none   # 不发送推理强度参数
