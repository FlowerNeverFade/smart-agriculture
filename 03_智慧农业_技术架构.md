...
User/Event
  ...
  -> deterministic rules
  -> hierarchical RAG + query/diagnose/plan tools
  -> Agent Orchestrator
  -> Safety Policy
...
| `crop-pack-module` | 作物包注册、版本、Schema 校验、阶段识别和参数继承 | `/crop-packs`、`crop-pack.*` |
...
| `crop_stage_profile` | crop_pack_id、stage_code、sequence、targets | 生长阶段与目标配置 |
| `crop_metric_profile` | crop_pack_id、stage_code、metric_code、range、availability | 动态指标和数据可用性 |
| `crop_batch` | id、plot_id、crop_code、stage_code、crop_pack_version | 地块当前作物上下文 |
...
| `decision_ledger` | event_id、before、decision、after、versions、actor、hash | 决策审计和回放 |
...
| 诊断 | `detect_anomaly`、`get_crop_profile`、`get_growth_stage` | 不直接控制执行器，不猜测不可用指标 |
