...
> 2026-08-24 本地静态预览登录降级修复：静态服务器接收 `/api/v1/auth/login` 的 POST 时会返回 404/405/501，现将这些状态与网络中断统一识别为“后端未接入”，仅在账号属于预置演示角色且密码为 `demo123` 时创建 DEMO 会话；401/鉴权失败仍明确拒绝，不伪装成登录成功。真实浏览器已验证 `admin/demo123` 可进入本地演示，并能打开“水资源协同排程”；页面存在 `.resource-balance-visual`，控制台无 error/warn。该证据只代表本地模拟模式，不代表 JWT 后端验收。

> 2026-08-25 农务功能分支同步最新主线：`feat/farm-operations` 已本地合入 `origin/main@095e256`，输入为主线最新首页、账户流程和麦田 WebGL 降级更新；合并处理中保留农务浅绿色主题、水资源资源位及静态预览登录降级，输出为同一功能分支中的最新 Home 与农务模块。全部 Web UI JavaScript 语法检查和 CSS 括号检查通过；真实浏览器验证 Home 麦田背景、浅色农务沙盘和水资源入口，控制台无运行时 error；仅有 Three.js `Clock` 弃用提示，不影响当前展示与交互。`jsdom` 回归因本地依赖未安装未执行，不将其计作通过证据。

> 2026-08-25 Home 简洁版：在独立分支 `codex/simple-home` 中将首屏收口为 4 项核心指标、动态地块示意图、最高 2 项风险、3 项今日农务和水资源进度；Home 隐藏地块筛选和右侧系统栏，进入功能模块后恢复完整布局。Copilot 与关键动态改为可聚焦弹层，未复制业务处理；深浅主题均保留原麦田背景，并以不同透明度保证文字对比度。专项真实 Chromium 验证 `13/13`，覆盖主题、导航展开、弹层、地块路由、水资源入口、窄屏和运行时错误；证据为 `scripts/simple-home-smoke.mjs` 与 `artifacts/simple-home/` 本地截图。本次只代表前端演示切片，未改动后端或宣称真实硬件效果。
...
| 账号管理 | 已完成（公网验收） | 100%（当前功能范围） | 登录/注册身份选择、服务端角色核验、普通身份安全注册、管理员自助注册阻断、恢复码重置、BCrypt 哈希、失败限流及旧 JWT 失效均通过；Flyway v2/v3、远端登录/注册/角色黑盒和公网登录页已验收，见 `docs/account-management.md` |
| 农务执行前端（独立功能分支） | 已完成（前端集成验收） | 100%（演示切片） | 四态工单、巡田证据、农田动态画布和 WebGL2 水务 Shader 已合入并发布；透明农田沙盘保持原有结构与动画，浅色主题改为低饱和浅绿色背景及浅色任务面板；水球已收纳到余量卡片资源位，约 58 KB WebP 网格背景按昼夜主题移动，鼠标拖尾/点击水花保留全窗口响应；资源/效果明确标记 SIMULATED；Node 三模式回归、真实 Chromium 和公网 JWT 浏览器复核均通过，不计作真实现场效果 |
