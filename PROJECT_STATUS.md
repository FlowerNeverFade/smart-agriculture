...
> 2026-08-25 农务功能分支同步最新主线：`feat/farm-operations` 已本地合入 `origin/main@095e256`，输入为主线最新首页、账户流程和麦田 WebGL 降级更新；合并处理中保留农务浅绿色主题、水资源资源位及静态预览登录降级，输出为同一功能分支中的最新 Home 与农务模块。全部 Web UI JavaScript 语法检查和 CSS 括号检查通过；真实浏览器验证 Home 麦田背景、浅色农务沙盘和水资源入口，控制台无 error/warn。`jsdom` 回归因本地依赖未安装未执行，不将其计作通过证据。
