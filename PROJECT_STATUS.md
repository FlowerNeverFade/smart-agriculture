...
> 2026-08-26 BearPi E53_IA1 实时接入适配（本地硬件与远端链路已验收）：新增 `hardware/bearpi_e53_bridge.py` 与一键启动器 `hardware/connect_bearpi.py`，支持官方 E53 串口文本和 JSON 读数，发布温度/空气湿度/光照并带 `REAL/HARDWARE/OBSERVED` 来源；后端新增来源持久化、120 秒真实优先仲裁和空气湿度指标，模拟器改为平滑温湿度/昼夜光照/水位轨迹并每 5 秒持续发送。已在 COM5 刷写 E53_IA1 固件，HiBurn 全部分区日志显示 `Execution Successful`，串口连续读到变化中的光照/湿度/温度；通过 SSH 隧道发布到服务器 MQTT 后，远端 `/api/v1/plots/plot-a01/telemetry` 返回 `sourceMode=REAL`、`dataOrigin=HARDWARE`。一键命令为 `py hardware/connect_bearpi.py --port COM5 --plot-id plot-a01`（密码交互输入）；GitHub `main` 与 `/srv/agriloop` 当前均为 `b1048ef`，服务器本机 acceptance smoke PASS，公网健康/SSE/三指标查询通过。Python 解析、桥接器、Gradle API、前端 Node、Vite、Crop Pack 和语法检查通过。该项是用户明确追加的硬件适配验收，不把生产现场网关/GPIO/执行器误写成软件基线已完成。
