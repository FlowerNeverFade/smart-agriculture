...
> 2026-08-26 地块级模拟策略增强（本地+远端已验收）：模拟器按地块热加载独立场景和有界随机波动，支持正常/干旱/暴雨/传感器漂移/设备离线；风险预测与灌溉建议复用策略参数，地块详情提供参数滑块、即时预测预览和历史/预测重置，硬件 `REAL/HARDWARE` 数据按地块优先且重置不删除。风险与农户 SVG 曲线支持鼠标局部数据浮窗。Gradle API、Python simulator、Node 19/19、Vite、stub/svg/real 三种 Web 回归和 OpenAPI YAML 解析通过；远端 `main@2c2fd44` 已部署，a01 干旱与 a02 暴雨黑盒验证显示温湿度/土壤湿度/降雨量反向变化，硬件离线安全门和缓存版本号也已验证。详见 `docs/simulation-strategies.md`。
...
> 2026-08-26 BearPi E53_IA1 实时接入适配（本地硬件与远端链路已验收）：新增 `hardware/bearpi_e53_bridge.py` 与一键启动器 `hardware/connect_bearpi.py`，支持官方 E53 串口文本和 JSON 读数，发布温度/空气湿度/光照并带 `REAL/HARDWARE/OBSERVED` 来源；后端新增来源持久化、120 秒真实优先仲裁和空气湿度指标，模拟器改为平滑温湿度/昼夜光照/水位轨迹并每 5 秒持续发送。已在 COM5 刷写 E53_IA1 固件，HiBurn 全部分区日志显示 `Execution Successful`，串口连续读到变化中的光照/湿度/温度；通过 SSH 隧道发布到服务器 MQTT 后，远端 `/api/v1/plots/plot-a01/telemetry` 返回 `sourceMode=REAL`、`dataOrigin=HARDWARE`。一键命令为 `py hardware/connect_bearpi.py --port COM5 --plot-id plot-a01`（密码交互输入）；运行时代码由发布提交 `b1048ef` 验收，服务器本机 acceptance smoke PASS，公网健康/SSE/三指标查询通过，之后的提交只同步本条验收记录。Python 解析、桥接器、Gradle API、前端 Node、Vite、Crop Pack 和语法检查通过。该项是用户明确追加的硬件适配验收，不把生产现场网关/GPIO/执行器误写成软件基线已完成。
...
> 2026-08-26 `farmer-ui` P1 农户呈现增量（待验收）：首页新增紧凑的未来天气/风险与设备处理卡，设备能力未加入左侧导航；地块详情增加 Crop Pack 批次阶段时间线，灌溉建议增加只读水量分配、相似案例与人工确认，顶部增加 MOCK/预测不可用降级横幅，头像弹窗增加日报周报入口。输入来自当前地块、工单、Crop Pack、风险预测及资源计划；接口不可用时明确使用 `SIMULATED/MOCK/UNAVAILABLE`，反馈不会静默修改策略。已通过 `node --check`、`git diff --check` 和本地 HTTP 200 静态资源检查；受浏览器本地地址策略限制，本轮视觉及完整点击回归仍待人工验收。
...
| 农户端 P1 增量呈现 | 待验收 | 页面实现完成 | I-16/I-27 天气风险、I-18/I-26 设备核验、I-19 批次阶段、I-22 水量分配、I-24 降级提示、I-25 报告入口、I-23/I-29 案例与人工确认；当前仅完成静态与语法检查，需补桌面/窄屏和浅深色截图证据 |
