...
### 2026-08-26 / AI 告警处置与管理员对话助手

- 今日完成：农场管理员导航改为“AI 告警处置”；告警列表增加全选、批量直接下发、批量关闭和 AI 智能处理，删除“确认收到”、“升级处理”和“转成任务”的多余中间步骤。
- 处理链路：输入为地块告警、后端确定性诊断、农户地块权限和任务记录；置信度达标后按“有权限→当前任务少→该地块经验多”选择农户并直接下发；缺失证据、候选告警或无合适农户时保留人工审核。
- 失败与兼容：已有关联任务不重复创建；历史未分配任务仍可由 AI 继续派单；单条失败不影响其他批量项，并显示需人工处理数量。
- 对话页：农场管理员原“智能诊断与灌溉”二级页替换为完整 AI 对话助手，支持地块上下文、历史对话、快捷问题、新对话和模型降级来源提示；不改动农户与系统管理员原有页面。
- 可复现证据：Web 单元测试 `20/20`、Vite 生产构建通过、Gradle API 回归通过；Edge 正式管理员登录验证新文案/批量按钮/旧入口移除、AI 对话真实请求与 `rules-only` 降级回答，页面运行错误为 0。
- 当前状态：待用户本地界面验收；未推送远端。
