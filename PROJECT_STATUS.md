...
> 2026-08-24 yyx1 分支前端重写：按 newUI 设计稿完成 Google Material 风格前端，旧仪表盘 UI 移除，登录页保留 main 版（WebGL 流体背景）不动。新单页工作台含今日/地块/农务/风险/经营五视图与可收回系统栏，登录后侧栏使用桌面新版透明底 logo；技术栈改用 Vue 3（vendored 全局构建）。数据策略：在线全部来自 /api/v1 后端合同，离线回落 main 模拟数据（显式 SIMULATED 口径，顶部琥珀条标注）。浏览器实测门禁/演示登录/退出闭环通过，证据 rtifacts/vue-*.png、rtifacts/newui-*.png。该重写仅在 yyx1 分支，未合入 main/yyx。
