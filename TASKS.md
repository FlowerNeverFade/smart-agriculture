...
> 2026-08-26 管理员第 2/3 轮本地闭环：八项管理能力、统一农场上下文与失效刷新、A→B 巡田证据、B→A 补证工单、Crop Pack→生产计划及三角色回归均已完成；正式数据不使用 Mock 补齐，模拟设备/虚拟执行边界不变。本轮只创建本地提交，不推送或部署。
...
| T-053 | P0 | 管理员第 2 轮八项管理能力：设备、地块生命周期、成员权限、生产计划、农场切换、Crop Pack、价值账本和模拟器 | 前后端 | — | 本轮 | 已完成（本地验收） | 后端角色/农场/地块校验、地块依赖保护、设备离线事实、计划幂等与来源、正式数据空值边界；前端固定页签、统一上下文和八项浏览器闭环；详见 `docs/acceptance/ADMIN_ROUND_2_3_ACCEPTANCE.md` |
| T-054 | P0 | 管理员第 3 轮统一接线与三角色回归 | 前后端 | — | 本轮 | 已完成（本地验收） | A→B 人工巡田证据进入诊断/护照，B→A 补证任务进入统一工单，Crop Pack 模板进入可执行工单生命周期；农户和系统管理员无管理员页签；状态测试、Gradle、Vite 与浏览器回归通过 |
| T-055 | P0 | 三角色统一账号资料面板与角色化概览 | 前端 | — | 本轮 | 已完成（本地验收） | `account-profile.js`/`account-profile.css` 共用资料结构；农户显示农务统计、农场管理员显示运营范围与告警/任务/成员/设备、系统管理员显示服务/审计/规则/模拟器；三角色 Chromium、密码操作、桌面/390px 窄屏、`git diff --check` 与 `npx vite build` 通过；本地提交，未推送 |
...
| T-061 | P0 | 合并农场管理员总览与地块管理页面 | 前端 | — | 本轮 | 待验收 | 移除首页二级页签；今日概况、全部地块、状态/编辑入口和添加地块卡片同屏展示；Web 15/15、Vite 构建、Chromium 7→8 新增闭环通过，待用户本地检查 |
| T-062 | P0 | 修复设备绑定显示并补齐种植农户成员增删改 | 前后端 | — | 本轮 | 待验收 | 设备绑定结果即时回写且绑定/在线状态分开展示；总览在无首次心跳时显示“设备已绑定，等待首次数据”；成员页取消重复权限页签，地块范围归入新增/修改窗口，添加卡使用加号；后端 38/38、Web 16/16、Vite、Chromium 正式绑定与成员页面回归通过，待用户本地检查 |
