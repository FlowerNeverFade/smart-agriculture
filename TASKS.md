...
>
> 当前 T-061、T-062、T-063、T-065、T-066、T-067 等待用户界面验收；T-064 已完成。默认不做视觉检测或自动截图；完成时仅在没有本地页面打开的情况下打开，已有页面时不重复打开。当前证据入口见 `docs/README.md`，已结束的分工计划、旧 3D 页面验收和历史截图由 Git 历史保存。
>
...
| T-066 | P0 | 告警冷却生效、高温告警、在线改密、成员创建/停用与巡田照片附件 | 前后端 | — | 本轮 | 待验收 | 冷却期复用活动告警、`HEAT_STRESS` 状态机、`POST /auth/change-password`、成员创建/启停及最多 6 张 `USER_PROVIDED` 本地附件；远端记录 Gradle API 43/43、前端 Node 14/14、Vite 构建通过，待本地回归与用户验收 |
| T-067 | P1 | 管理员指标中文化、逾期任务页签与告警降级处理修复 | 前端 | — | 本轮 | 待验收 | 已知指标显示简洁中文；任务页“已逾期”进入 `scope=overdue` 并高亮；已升级告警复用冻结的 `ack` 合同降为“已确认”。Web 21/21、Vite 构建和 `git diff --check` 通过；待用户在本地页面确认 |
