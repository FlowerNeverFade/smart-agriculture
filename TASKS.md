...
>
> 当前 T-061、T-062、T-063、T-065、T-066、T-067、T-068、T-069 等待用户界面验收；T-064 已完成。默认不做视觉检测或自动截图；完成时仅在没有本地页面打开的情况下打开，已有页面时不重复打开。当前证据入口见 `docs/README.md`，已结束的分工计划、旧 3D 页面验收和历史截图由 Git 历史保存。
>
...
| T-067 | P1 | 管理员指标中文化、逾期任务页签与告警降级处理修复 | 前端 | — | 本轮 | 待验收 | 已知指标显示简洁中文；任务页“已逾期”进入 `scope=overdue` 并高亮；已升级告警复用冻结的 `ack` 合同降为“已确认”。Web 22/22、Vite 构建和 `git diff --check` 通过；待用户在本地页面确认 |
| T-068 | P1 | 管理员农务任务概要卡网格与详情弹窗 | 前端 | T-064/T-067 | 本轮 | 待验收 | 仅 `FARM_ADMIN` 使用 `auto-fill/minmax(280px, 1fr)` 概要卡；点击或键盘打开居中详情并复用分配、验收、取消流程，农户和系统管理员原布局不变。`node --check`、Web 22/22、Vite 构建及 `git diff --check` 通过；按规则不做自动视觉验收，待用户本地检查 |
| T-069 | P1 | 管理员告警与设备概要卡网格及详情弹窗 | 前端 | T-063/T-067/T-068 | 本轮 | 待验收 | 仅 `FARM_ADMIN` 的告警处置和设备管理使用 `auto-fill/minmax(280px, 1fr)` 概要卡；告警操作、关联任务、设备注册/绑定/解绑均复用现有合同，设备注册改为添加卡片。`node --check`、Web 23/23、Vite 构建和 `git diff --check` 通过；按规则不做自动视觉验收，待用户本地检查 |
