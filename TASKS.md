...
| T-074 | P1 | 管理员农务任务生命周期、类型与重点关注分类重构 | 前端 | T-068/T-069/T-071 | 本轮 | 待验收 | 仅 `FARM_ADMIN` 将顶层任务概况改为待分配、待执行、执行中、待验收、需返工、已结束六个互斥阶段；任务类型与逾期/今日到期/未来 7 天/高优先级作为独立筛选，来源移至卡片底部，未知任务类型保留后端原值。Node、Web 26/26、Vite 与 `git diff --check` 已通过；浏览器实测待分配、设备检查、高优先级、`scope=overdue`、Enter/Esc，以及桌面四列与窄屏单列无页面横向溢出；不修改接口、权限、Hash 范围或其他角色页面，待用户确认分类清晰度 |
