...
| T-048 | P0 | 修复静态预览服务器返回 404/405/501 时的离线演示登录降级 | 前端 | — | D15+ | 已完成 | 输入为静态服务器 API 错误；仅固定演示账号和 `demo123` 创建 DEMO 会话，401 及错误密码继续拒绝；真实浏览器验证登录、水资源入口和零控制台错误 |
| T-049 | P1 | 建立 Home 低密度简洁版及独立试验分支 | 前端 | — | D15+ | 已完成 | `codex/simple-home`；动态地块、风险/农务/用水聚合、Home 专属布局、Copilot/动态弹层、深浅主题；`scripts/simple-home-smoke.mjs` 真实 Chromium 13/13 |
