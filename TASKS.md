...
| T-044 | P1 | yyx1 分支 Google Material 风格前端重写（newUI 设计稿，Vue 3） | 前端 | — | D15+ | 已完成 | 旧仪表盘移除、登录页保留 main 版；Vue 3 单页工作台（今日/地块/农务/风险/经营 + 可收回系统栏）；登录后使用桌面新版透明底 logo；在线数据唯一来源 /api/v1，离线回落 main 模拟数据（SIMULATED 标注）；浏览器实测门禁/演示登录/退出闭环通过，证据 rtifacts/vue-*.png；仅 yyx1 分支 |
