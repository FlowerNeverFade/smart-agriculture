...
| T-075 | P0 | AI 告警分析、统一智能处理与管理员经典对话页 | 前端 | T-056/T-068/T-070 | 本轮 | 待验收 | 入口保持“AI告警分析与智能处理”；页面仅保留全选、AI智能处理和批量关闭，移除重复的一键下发/详情下发、“确认收到”、“升级处理”和“转成任务”。AI 按置信度/缺失证据/规则状态判定后，对高可信告警选择合适农户并直接创建/分配工单；不确定项保留审核。派单响应缺字段时会补齐本地状态；关闭状态写回演示数据源并兼容 `id`/`alertId`，批量或单条关闭后卡片立即消失且刷新不回弹。告警与对话模块大标题已移除；AI 对话页改为经典单列消息、中央四个快捷问题和底部圆角输入框。本地预览不依赖后端登录。Node 37/37、Vite、Web real/stub/svg 25/25/24 通过；待用户本地视觉与交互验收。 |
