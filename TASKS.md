...
> 项目周期：15 天
> 任务版本：v2.6（2026-08-26）
> 状态枚举：`未开始` / `进行中` / `待验收` / `已完成` / `阻塞`

> 当前远端功能基线为 `origin/main@45918f6`；本地 `main` 已普通合并远端新增提交，并保留未推送的文档整理与管理员修复。远端新增告警冷却、高温告警、在线改密、成员生命周期、巡田照片与农户 P1 呈现；前端演示结果不代表真实现场效果。
>
> 当前 T-061、T-062、T-063、T-065、T-066、T-068、T-069、T-070、T-071、T-073、T-074、T-075、T-076、T-077 等待用户界面验收；T-067 仍在进行，T-064 已完成。界面改动在代码、自动化测试和构建完成后统一执行一次浏览器功能与视觉验收，提供功能清单和必要截图；复用已有页面，不在开发过程中反复截图。当前证据入口见 `docs/README.md`，已结束的分工计划、旧 3D 页面验收和历史截图由 Git 历史保存。
>
...
| T-065 | P1 | 农户端天气风险、设备核验、批次阶段、资源分配、降级、报告和案例确认呈现 | 前端 | T-027/T-028/T-034/T-035/T-036 | 本轮 | 待验收 | `farmer.html/js/css` 已实现；输入为地块/工单/Crop Pack/预测/资源计划，失败路径显示 `MOCK/SIMULATED/UNAVAILABLE` 且不直接控制设备或发布策略；远端记录 `node --check`、`git diff --check`、HTTP 200 通过，待用户验收 |
| T-066 | P0 | 告警冷却生效、高温告警、在线改密、成员创建/停用与巡田照片附件 | 前后端 | — | 本轮 | 待验收 | 冷却期复用活动告警、`HEAT_STRESS` 状态机、`POST /auth/change-password`、成员创建/启停及最多 6 张 `USER_PROVIDED` 本地附件；远端记录 Gradle API 43/43，本地合并后前端 22/22 与 Vite 构建通过，后端未复跑，待用户验收 |
...
| T-068 | P1 | 管理员指标中文化、逾期任务页签与告警降级处理修复 | 前端 | — | 本轮 | 待验收 | 已知指标显示简洁中文；任务页“已逾期”进入 `scope=overdue` 并高亮；已升级告警复用冻结的 `ack` 合同降为“已确认”。Web 22/22、Vite 构建和 `git diff --check` 通过；待用户在本地页面确认 |
| T-069 | P1 | 管理员农务任务概要卡网格与详情弹窗 | 前端 | T-064/T-068 | 本轮 | 待验收 | 仅 `FARM_ADMIN` 使用 `auto-fill/minmax(280px, 1fr)` 概要卡；点击或键盘打开居中详情并复用分配、验收、取消流程，农户和系统管理员原布局不变。`node --check`、Web 22/22、Vite 构建及 `git diff --check` 通过；按规则不做自动视觉验收，待用户本地检查 |
...
| T-071 | P1 | 管理员 macOS 层级化语义色视觉统一 | 前端 | T-062/T-069/T-070 | 本轮 | 待验收 | 仅 `FARM_ADMIN` 使用彩色导航、五项概况、指标图标和任务/告警/设备/成员语义色表面；任务列表嵌入管理页时隐藏重复大标题并取消独立居中宽度，使操作区、概况和筛选器与页签对齐。顶栏与侧栏保留轻材质，内容卡片保持清晰白色层级，不增加背景图、不改变接口和其他角色界面。Node、Vue 模板、Web 23/23 与 Vite 已通过；浏览器实测单一标题、区域对齐、任务卡 Enter/详情/Esc、“待分配”及 `scope=overdue` 筛选和控制台健康均通过，已提供页面与详情截图，待用户验收 |
| T-073 | P1 | 管理员生产计划与 Crop Pack 概要卡网格及详情弹窗 | 前端 | T-061/T-071 | 本轮 | 待验收 | 仅 `FARM_ADMIN` 将生产计划和 Crop Pack 常驻左右分栏改为 `auto-fill/minmax(280px, 1fr)` 概要卡；新建计划使用添加卡片，生成预览、日期调整、审批及 Pack 阶段/指标事实继续复用现有合同并进入居中弹窗。`node --check`、Web 23/23、Vite、`git diff --check` 通过；浏览器实测计划生成、详情打开、Crop Pack 中文指标及 Esc 关闭，待用户确认观感 |
| T-074 | P1 | 管理员农务任务生命周期、类型与重点关注分类重构 | 前端 | T-068/T-069/T-071 | 本轮 | 待验收 | 仅 `FARM_ADMIN` 将顶层任务概况改为待分配、待执行、执行中、待验收、需返工、已结束六个互斥阶段；任务类型与逾期/今日到期/未来 7 天/高优先级作为独立筛选，来源移至卡片底部，未知任务类型保留后端原值。宽屏操作按钮与页签同排并释放原占位，任务进度和“全部任务”整体上移；窄屏恢复纵向排列。Node、Web 26/26、Vite 与 `git diff --check` 已通过；浏览器实测待分配、设备检查、高优先级、`scope=overdue`、Enter/Esc，以及桌面四列与窄屏单列无页面横向溢出；不修改接口、权限、Hash 范围或其他角色页面，待用户确认分类与顶部布局 |
| T-075 | P1 | 管理员总览综合健康色阶与卡片状态解耦 | 前端 | T-062/T-071 | 本轮 | 待验收 | 综合健康数字只按分数自身映射良好、关注、危险和缺失色阶，不再继承地块卡片的设备/风险关注色；卡片顶边继续独立表达异常。Node 语法、Web 27/27、Vite 与 `git diff --check` 通过；浏览器实测 B02/B03 同为 88 且同色，B03 橙色关注顶边仍保留，72/52 分色阶正确，桌面和窄屏无横向溢出，待用户确认色彩语义 |
| T-076 | P1 | 管理员设备资产与资源安排职责重构 | 前端 | T-053/T-070/T-074 | 本轮 | 待验收 | 仅 `FARM_ADMIN` 将导航改为“设备与设施”，设备页增加全部/在线/需处理/未绑定概况、组合筛选、设备详情及关联告警/设备检查任务；用水容量试算和价值对账迁入农务任务“资源安排”，旧 `irrigation/value/simulator` 地址兼容回落，系统管理员仿真验证和其他角色界面不变。相关文件 `node --check`、Web 30/30、Vite 和 `git diff --check` 通过；浏览器实测设备组合筛选、Enter/Esc 详情、关联任务跳转、新建任务地块预选、用水不足试算结果保持、价值对账录入、旧地址回落及系统管理员仿真入口，桌面与窄屏无横向溢出，待用户确认职责划分与页面效果 |
| T-077 | P1 | 管理员 macOS 分组层级与中性材质重构 | 前端 | T-071/T-074/T-076 | 本轮 | 待验收 | 仅 `FARM_ADMIN` 将顶栏与侧栏收敛为轻量材质，导航统一使用品牌强调色；首页、任务和设备概况改为中性表面，仅保留图标及异常状态语义色。地块指标由连续表格改为独立分组单元，消除末行空槽；卡片阴影、圆角、按钮和弹窗层级统一，深色模式改善 Logo 对比，窄屏顶栏改为两行并完整显示农场选择器。Web 30/30、Vite 和 `git diff --check` 通过；浏览器实测首页、任务、设备、深色模式和 390px 窄屏，无页面横向溢出，待用户确认整体观感 |
