...
| T-073 | P1 | 地块详情多指标曲线与预测重置连续性修复 | 前后端 | T-067 | 本轮 | 待验收 | 地块详情支持土壤湿度、空气温度/湿度、光照、CO₂、pH、水位、降雨量八类曲线；每项使用独立单位/纵轴，历史与预测首点共享最后一条有效遥测的时间和值；重置预测重新读取当前指标并抑制正常场景短窗口噪声外推，演示模式补齐各指标模拟值。新增后端连续性回归和 OpenAPI 指标枚举；Node、Vite、Web stub/svg/real、Gradle 与 Python simulator 本地检查通过，待远端发布验收。 |
