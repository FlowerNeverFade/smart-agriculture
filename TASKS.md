...
> 本轮新增 T-072 农户首页任务优先级与建议安全闭环，已完成静态检查、Web 单元测试和本地浏览器交互回归，等待用户验收；灌溉确认在农户角色下只提交管理员审批，不直接控制水泵。
>
...
| T-072 | P0 | 农户首页任务优先级与建议安全闭环 | 前端 | T-011/T-014/T-028/T-033 | 本轮 | 待验收 | `farmer.html/js/css` 新增最多三项优先事项、四步建议状态、任务结果表单、灌溉管理员审批二次确认和恢复状态；天气/设备内部能力编号不再面向农户展示，首页地块仅呈现健康摘要，详情继续保留六项曲线。`submitDecisionFeedback` 复用反馈合同；农户不调用 `executeIrrigation`。`node --check`、Web 23/23、`verify-webui svg`、本地浏览器任务/灌溉/地块闭环已通过，待用户验收 |
