...
| T-049 | P0 | 农户专用工作台：首页简化、地块指标/历史、风险与灌溉建议、巡田/补证、账户自助 | 前端 | — | 本轮 | 已完成 | `apps/web-ui/farmer.html`、`farmer.js`、`farmer.css`；四项统计卡及任务/消息直达、3 个授权地块网格、六项指标 SVG 趋势子图与 7h/24h/7天范围、灌溉建议土壤湿度曲线、1/2/4 小时风险参考、演示灌溉、规则问答/紧凑反馈、巡田与补证表单；新增原有 `decision-console`、`risk-forecast`、`work-orders` 三个共享模块入口；本轮补充头像个人中心弹窗（原资料/统计/密码操作）和基于指标、设备新鲜度、风险等级的综合健康评分；合入 `dab54bb` 工具工作台（作物培养手册、工单巡田和共享模块）；本地 Chromium 已验证弹窗内容/关闭与密码入口、评分 68/87/79、地块导航、工具台进出、浅深色切换，运行时 error/warn 为 0。 |
