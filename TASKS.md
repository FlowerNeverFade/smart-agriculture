...
>
> 当前 T-061、T-062、T-063、T-065、T-066、T-068、T-069、T-070、T-071、T-075 等待用户界面验收；T-064、T-067、T-072、T-073、T-074 已完成。界面改动已完成自动化验证和构建，浏览器视觉细节仍待用户集中检查；复用已有页面，不在开发过程中反复截图。当前证据入口见 `docs/README.md`，已结束的分工计划、旧 3D 页面验收和历史截图由 Git 历史保存。
>
...
| T-075 | P0 | AI 告警分析、批量直接派单与管理员完整对话页 | 前端 | T-056/T-068/T-070 | 本轮 | 待验收 | 入口更名为“AI告警分析与智能处理”；保留 main 的 `auto-fill/minmax(280px, 1fr)` 概要卡和居中详情，增加全选、批量直接下发、批量关闭和 AI 智能处理，移除“确认收到”、“升级处理”和“转成任务”。AI 按诊断置信度/缺失证据/规则状态审核，高可信告警按农户地块权限、当前工作量和过往地块经验选人并直接创建/分配工单；不确定或无合格农户时保留人工审核。农场管理员原诊断/灌溉页替换为完整 AI 对话，正文与输入为 16px。本地 `?preview=farm-admin` 入口直接创建演示会话，不依赖后端登录。Node 34/34、Vite、Web real/stub/svg 24/24/23 通过；待用户本地视觉与交互验收。 |
