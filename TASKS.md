...
>
> 当前 T-061、T-062、T-063、T-065、T-066、T-068、T-069、T-070、T-071 等待用户界面验收；T-067 仍在进行，T-064 已完成。默认不做视觉检测或自动截图；完成时仅在没有本地页面打开的情况下打开，已有页面时不重复打开。当前证据入口见 `docs/README.md`，已结束的分工计划、旧 3D 页面验收和历史截图由 Git 历史保存。
>
...
| T-071 | P1 | 管理员 macOS 层级化语义色视觉统一 | 前端 | T-062/T-069/T-070 | 本轮 | 待验收 | 仅 `FARM_ADMIN` 使用彩色导航、五项概况、指标图标和任务/告警/设备/成员语义色表面；顶栏与侧栏保留轻材质，内容卡片保持清晰白色层级，不增加背景图、不改变接口和其他角色界面。`node --check`、Vue 模板编译、Web 23/23、Vite 构建和 `git diff --check` 通过；按规则不做自动视觉验收，待用户本地检查 |
