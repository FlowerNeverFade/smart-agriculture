...
| T-044 | P1 | yyx1 分支 Google Material 风格前端重写（newUI 设计稿） | 前端 | — | D15+ | 已完成 | 旧仪表盘移除、登录页保留 main 版；五视图单页工作台 + 可收回系统栏；登录后使用桌面新版透明底 logo；沿用 `/api/v1` 合同与离线 SIMULATED 降级；浏览器实测回归（首开/切换/重复折叠/下发）通过，证据 `artifacts/newui-*.png`；仅 yyx1 分支 |
