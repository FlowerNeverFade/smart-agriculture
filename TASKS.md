...
>
> 当前 T-061、T-062、T-063、T-065 等待用户界面验收；T-064 已完成。默认不做视觉检测或自动截图，完成后打开本地页面供用户检查。当前证据入口见 `docs/README.md`，已结束的分工计划、旧 3D 页面验收和历史截图由 Git 历史保存。
>
...
| T-065 | P1 | 管理员指标中文化、逾期任务页签与告警降级处理修复 | 前端 | — | 本轮 | 待验收 | 已知指标在管理员总览、地块详情、智能诊断、告警来源与 Crop Pack 阈值表中显示简洁中文；任务页“已逾期”进入 `scope=overdue` 并高亮；已升级告警复用冻结的 `ack` 合同降为“已确认”。Web 21/21、Vite 构建和 `git diff --check` 通过；待用户打开本地页面确认 |
