...
>
> 当前 T-061、T-062、T-063、T-065、T-066、T-068、T-069、T-070、T-071、T-075、T-076 等待用户界面验收；T-064、T-067、T-072、T-073、T-074 已完成。界面改动已完成自动化验证和构建，浏览器视觉细节仍待用户集中检查；复用已有页面，不在开发过程中反复截图。当前证据入口见 `docs/README.md`，已结束的分工计划、旧 3D 页面验收和历史截图由 Git 历史保存。
>
...
| T-076 | P0 | 农务任务五段状态流、返工标记与逾期批量处置 | 前端 | T-056/T-064/T-069 | 本轮 | 待验收 | 导航和任务状态下拉统一为进行中/待分配/待验收/已逾期/已完成；未提交的逾期执行任务优先分流并 30 秒自动刷新，提交后进入待验收，`ASSIGNED` 归入进行中，`REJECTED` 仅显示返工标记并保留原负责人。逾期页提供全选、一键按权限和负载重新分配、单任务选择人员处置；统一任务模块与主应用 API 实例，操作后负责人和状态立即更新。演示数据补充可验收逾期任务及第二位地块授权农户。Node 44/44、Vite、Web real/stub/svg 30/30/29 通过；待用户本地检查。 |
