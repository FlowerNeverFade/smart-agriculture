...
## 7. 2026-09-02 三角色正式数据首屏收口

- 农场管理员、系统管理员和种植农户先并行读取总览、地块、任务、告警和设备等核心数据；作物包、成员、账本、巡田历史、资源治理和助手会话在首屏后补齐。
- 每个核心请求采用约 2.2 秒预算。单个接口超时不会清空已显示的状态，后台轮询会再次对账；系统管理员治理详情以较低频率刷新，避免历史接口与实时遥测争用。
- SSE 握手移出路由渲染关键路径；入口脚本缓存查询版本同步刷新，发布后浏览器会加载本轮脚本。
- 降级路径：核心请求全部失败时保留原有错误提示和权限边界；慢/不可用的可选服务只显示已成功事实，不伪装为实时学习或执行结果。
- 本地证据：三个角色脚本 `node --check`、Vite 构建和受控学习后端定向测试已通过；GitHub 失败运行 `33546727838` 的根因已修复，待本轮推送后的 Actions 运行确认。
