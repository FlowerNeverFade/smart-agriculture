...
> 最近验收日期：2026-09-03
> 功能编号：T-134（排序基线）/ T-152（V5.9.20 拖动可用性修复）
> 范围：本地 `main` 预览工作树；V5.9.20 未推送、未部署。既有排序后端基线曾随 `165aefd9` 发布，本轮不修改接口、权限或服务器。
...
- 排序手柄支持方向键、Home 和 End；鼠标、触屏或键盘每次换位只保存一次，Escape、指针取消、版本冲突和保存失败均回滚。
...
- 初始卡片顺序为 `plot-a01 → plot-a02`；按住第一张卡的六点手柄直接拖到第二张卡后变为 `plot-a02 → plot-a01`，页面显示“地块排列已保存”。
- 刷新页面后顺序仍为 `plot-a02 → plot-a01`。
- 聚焦“温室2”手柄并按右方向键可恢复 `plot-a01 → plot-a02`，操作后焦点仍停留在同一手柄；再次刷新仍保留恢复后的顺序。
