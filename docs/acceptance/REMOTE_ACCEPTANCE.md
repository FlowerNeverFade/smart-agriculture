...
- 发布提交：`8c2c8c9`；服务器 `/srv/agriloop/DEPLOYED_COMMIT` 与该提交一致。
- 服务状态：`agriloop-api`、`agriloop-simulator`、`agriloop-nginx`、`agriloop-cron`、`qwen-vllm` 均为 `RUNNING`；`GET /actuator/health` 返回 `{"status":"UP"}`。
- 地块详情曲线：通过正式 JWT 会话检查 `plot-a01` 的 `SOIL_MOISTURE`、`AIR_TEMPERATURE`、`AIR_HUMIDITY`、`LIGHT`、`CO2`、`PH`、`WATER_LEVEL`、`RAINFALL` 八项指标；每项遥测与预测接口均返回，历史末点和预测 0 分钟点的时间/值连续。
- 重置交互：点击“重置预测曲线”后重新生成预测，首点仍与当前历史末点连续；未出现 `Date.now()` 造成的时间漂移或正常场景短窗口斜率放大。
- 浏览器控制台：上述远端切换和重置回归未捕获页面错误；静态入口已刷新 `20260826-multimetric-curve-fix` 缓存版本。
...
| main 公网 Web/API | PASS | 历史验收发布提交 `3cdf4b7`（包含右栏功能修复 `7d33092`）；当前发布提交为 `8c2c8c9`，服务入口和 JWT API 保持不变 |
...
远端历史收口通过公网域名复跑了 `scripts/acceptance_smoke.py`。脚本为六类指标各写入两条稳定且可审计的 `GOOD` 上下文数据，不清库、不改规则、不绕过安全门；正常模拟器保持运行。最终输出 `status=PASS`、`duplicateTelemetry=true`、`diagnosis=WATER_DEFICIT`、`readiness=READY`、失败命令效果 `INCONCLUSIVE`，决策护照可按同一 `traceId` 查询。另行复测漂移硬门及 Qwen：漂移不可执行；模型返回 `adapter=openai-compatible`、`llm.model=agriloop-qwen38-agri`、`degraded=false`。该段记录对应历史提交 `3cdf4b7`；当前多指标曲线发布提交为 `8c2c8c9`，详见上方 2026-08-26 验收段。
