...
> 本期交付边界：模拟数据传输 + BearPi E53_IA1 实时遥测与板载风扇/补光灯受控执行 + 智能体 + 可视化三主线及联调。
> BearPi 串口 -> MQTT -> API -> SSE 的真实遥测链路，以及 API -> MQTT -> 串口 -> GPIO -> 固件 ACK 的板载执行链路已纳入当前联调/验收；真实水泵/阀门、其他现场网关与执行器、鸿蒙端应用和生产级端-智-云联调仍单独列为后续范围。

## 当前硬件接入状态（2026-09-03）

- **已接入并验证**：BearPi HM Nano E53_IA1 通过串口桥接温度、空气湿度和光照，发布到 MQTT 后由 API 接收并推送到页面；事件来源为 `REAL/HARDWARE`，同地块同指标的真实读数优先于模拟读数。
- **已受控执行并验证**：仅对设备 `bearpi-e53-ia1-a01` 开放板载风扇和补光灯。农场管理员可手动控制，也可绑定高温（35°C 开、33°C 关）和白天缺光（50 lux 开、60 lux 关）告警；开启需要新鲜且质量合格的真实遥测，最终状态以固件 ACK 为准。
- **继续使用模拟**：未接入的指标、情景波动、预测和安全回放由进程内 `SimulationEngine` 提供，来源标为 `SIMULATION`。
- **仍未覆盖**：真实水泵/阀门、除上述扩展板之外的 GPIO/串口执行器、其他现场网关与传感器、鸿蒙端应用，以及完整生产级端-智-云控制闭环。灌溉命令当前仍是虚拟执行，并不会伪装成真实动作。
...
BearPi HM Nano E53_IA1 的实时适配器见 [`docs/hardware/bearpi-e53-ia1.md`](docs/hardware/bearpi-e53-ia1.md)。它通过串口把温度、空气湿度和光照桥接到 MQTT，并标记 `REAL/HARDWARE`；真实读数在后端优先于同指标模拟值，缺少的指标仍由模拟器补齐。适配器还把白名单风扇/补光指令交给固件执行，并等待 GPIO 状态 ACK。该证据只覆盖当前 BearPi 扩展板，不能外推为真实水泵、阀门或所有现场设备均已完成。
...
- 当前版本已完成 BearPi E53_IA1 的真实遥测、来源仲裁及该扩展板风扇/补光灯 GPIO 受控执行，并支持 Qwen 原生图片分析；仍未完成的范围包括真实水泵/阀门、其他 GPIO/串口执行器、其他现场传感器/网关、鸿蒙端应用，以及生产级端-智-云全链路和生产级视觉/语音能力。上述未覆盖设备仍沿用 `SIMULATION` 或虚拟执行，并作为后续 `DeviceGatewayDataSource`/执行器适配方向。
