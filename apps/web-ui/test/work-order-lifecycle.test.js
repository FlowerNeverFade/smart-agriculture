...
test('农务任务与主应用复用同一 API 数据实例并定时刷新逾期分区', () => {
  ...
  assert.match(lifecycleSource, /from '\.\/api\.js\?v=20260831-sync-v1'/);
  assert.match(appSource, /from '\.\/api\.js\?v=20260831-sync-v1'/);
  assert.match(managementSource, /from '\.\.\/api\.js\?v=20260831-sync-v1'/);
