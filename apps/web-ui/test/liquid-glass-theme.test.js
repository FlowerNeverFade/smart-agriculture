...
test('farm admin restores the readable sage liquid glass workspace', async () => {
  ...
  assert.match(adminCss, /V5\.8\.1 FARM_ADMIN Liquid Glass restoration/);
  assert.match(adminCss, /--admin-glass-surface:\s*rgba\(248, 252, 249, \.68\)/);
  ...
  assert.match(index, /admin-resource-planning\.css\?v=20260829-v581-liquid-glass/);
  assert.match(index, /admin\.css\?v=20260829-v581-liquid-glass/);
});
