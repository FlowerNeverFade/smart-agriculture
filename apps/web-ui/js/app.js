...
const AdminRulesView = {
  ...
  setup(props) {
    ...
    const packGridRef = ref(null);
    const gridColumnsCount = ref(1);
    let resizeObserver = null;

    Vue.onMounted(() => {
      resizeObserver = new ResizeObserver(entries => {
        for (let entry of entries) {
          let cols = Math.floor((entry.contentRect.width + 16) / 296);
          if (cols < 1) cols = 1;
          if (gridColumnsCount.value !== cols) gridColumnsCount.value = cols;
        }
      });
      if (packGridRef.value) resizeObserver.observe(packGridRef.value);
    });
    ...
    const packColumns = Vue.computed(() => {
      const cols = Array.from({ length: gridColumnsCount.value }, () => []);
      props.state.adminCropPacks.forEach((pack, index) => {
        cols[index % gridColumnsCount.value].push(pack);
      });
      return cols;
    });
    ...
    return { activeTab, expandedPacks, togglePack, showPackModal, editingPackId, packForm, cropIcons, expandedKnowledge, openCreatePack, openEditPack, savePack, deletePack, togglePackStatus, addStage, removeStage, addKnowledgeDoc, removeKnowledgeDoc, toggleKnowledge, packColumns, packGridRef };
  }
};
