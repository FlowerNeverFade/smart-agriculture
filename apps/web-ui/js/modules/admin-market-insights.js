...
export const AdminMarketInsightsView = {
  ...
  setup(props) {
    ...
    const axisDisclosure = computed(() => selectedHistory.value.length > 1
      ...
      : '真实历史曲线由系统逐日归档；当前样本不足时只显示已有报价，不补造历史。');
    ...
    const renderChart = async () => {
      ...
      chart.setOption({
        ...
        aria: { enabled: true, description: `${cropName}${rangeDays.value}日市场价格折线，单位元每公斤。缺失日期保留为空白。` },
        title: hasObservedPrice ? undefined : { text: selectedCrop.value?.available ? '历史行情正在积累' : '当前没有可用报价', left: 'center', top: 'middle', textStyle: { color: '#777777', fontSize: 14, fontWeight: 'normal' } },
        tooltip: {
          trigger: 'axis', confine: true, backgroundColor: '#ffffff', borderColor: '#8f8f8f', borderWidth: 1,
          textStyle: { color: '#262626', fontFamily: 'STIX Two Text, STIXGeneral, Times New Roman, DejaVu Serif, serif' },
          valueFormatter: value => value == null ? '缺失' : `${Number(value).toFixed(2)} 元/公斤`
        },
        legend: {
          show: hasObservedPrice, top: 8, right: 12, data: ['参考价', '市场最低', '市场最高'],
        ...
        yAxis: {
          type: 'value', scale: true, name: '元/公斤', nameGap: 14,
    ...
    const chooseCrop = cropCode => { selectedCropCode.value = cropCode; void renderChart(); };
    ...
    return {
      loading, error, market, scope, rangeDays, selectedCropCode, chartEl, farmId, crops, selectedCrop, source, status,
      sourceTone, selectedQuotes, selectedHistory, chartHistory, availableCount, axisDisclosure, RANGE_OPTIONS,
      price, signed, changeTone, sourceLabel, chooseCrop, chooseRange, chooseScope, refresh, observationTone, quoteDifference
    };
  },
  template: `
    <section class="market-insights" aria-labelledby="market-insights-title">
      ...
      <div class="market-toolbar" aria-label="行情范围">
        ...
        <span>单位统一为元/公斤；上涨、下跌同时使用文字与箭头标识。</span>
      </div>
        ...
        <div v-if="selectedCrop" class="market-terminal">
          <article class="market-chart-panel">
            <header class="market-panel-heading">
              ...
              <div class="market-range-switch" aria-label="历史范围">
            ...
            <div ref="chartEl" class="market-price-chart" role="img" :aria-label="selectedCrop.cropName + rangeDays + '日市场价格曲线'"></div>
