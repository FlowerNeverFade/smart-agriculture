...
export const AdminRulesStrategiesView = {
  ...
  setup(props, { emit }) {
    ...
    return {
      activeTab, busy, loadError, farmId, filteredRules, filteredCandidates, learningCases, filteredLearningCases, learningFilter, learningCounts, learningBusyId,
  ...
  template: `
    <section class="admin-governance-page" aria-labelledby="admin-governance-title">
      ...
      <section v-else-if="activeTab === 'candidates'" class="admin-governance-section" aria-label="农场策略候选集">
        ...
        <div v-if="filteredCandidates.length" class="admin-candidate-evidence-list"><article v-for="candidate in filteredCandidates" :key="'evidence-' + candidateId(candidate)" class="admin-candidate-evidence"><div><strong>{{ candidateId(candidate) }}</strong><span>引用案例：{{ offlineEvidence(candidate) }}</span></div><div><span>基线：{{ strategySummary(candidate.baselineStrategy) }}</span><span>候选：{{ strategySummary(candidate.proposedStrategy) }}</span></div><small v-if="String(candidate.status || '').toUpperCase() === 'ROLLED_BACK'">回滚记录：{{ rollbackSummary(candidate) }}</small></article></div>
        <div v-if="filteredCandidates.length" class="admin-table-wrap admin-governance-table-wrap"><table class="admin-table"><thead><tr><th>候选方案</th><th>学习来源与特征</th><th>案例数量</th><th>一致率</th><th>离线验证</th><th>状态</th><th>操作</th></tr></thead><tbody><tr v-for="(candidate, index) in filteredCandidates" :key="candidateId(candidate)"><td><strong class="admin-governance-code">候选方案 {{ index + 1 }}</strong><small>{{ candidate.createdAt ? String(candidate.createdAt).slice(0, 16).replace('T', ' ') : '—' }}</small><details v-if="candidate.evidenceCaseIds?.length" class="admin-governance-evidence"><summary>查看引用案例</summary><span>{{ candidate.evidenceCaseIds.join('、') }}</span></details></td><td><span class="status-pill agent">{{ sourceLabel(candidate) }}</span><small class="admin-governance-description">{{ candidateDescription(candidate) }}</small></td><td>{{ evidenceCount(candidate) }} 条</td><td>{{ consistency(candidate) }}</td><td><span class="status-pill" :class="statusTone(candidate.offlineValidation?.status || candidate.validationStatus || candidate.status)">{{ statusLabel(candidate.offlineValidation?.status || candidate.validationStatus || candidate.status) }}</span><small v-if="offlineValidation(candidate).seed">随机种子 {{ offlineValidation(candidate).seed }} · 回放 {{ offlineValidation(candidate).replayHash || '—' }}</small><small v-if="offlineValidation(candidate).failures?.length">失败：{{ offlineFailures(candidate) }}</small></td><td><span class="status-pill" :class="statusTone(candidate.status)">{{ statusLabel(candidate.status) }}</span><small v-if="candidate.approvedBy">批准人：{{ candidate.approvedBy }} · {{ candidate.approvedAt || '—' }}</small></td><td class="admin-governance-actions"><button v-if="canValidate(candidate)" type="button" class="g-btn text sm" :disabled="busy" @click="validateCandidate(candidate)">执行离线验证</button><button v-if="canApprove(candidate)" type="button" class="g-btn text sm" :disabled="busy" @click="transition(candidate, 'APPROVED')">批准</button><button v-if="canActivate(candidate)" type="button" class="g-btn text sm" :disabled="busy" @click="transition(candidate, 'ACTIVE')">启用</button><button v-if="canReject(candidate)" type="button" class="g-btn text sm danger-text" :disabled="busy" @click="transition(candidate, 'REJECTED')">拒绝</button><button v-if="canRollback(candidate)" type="button" class="g-btn text sm danger-text" :disabled="busy" @click="transition(candidate, 'ROLLED_BACK')">回滚</button><span v-if="!canValidate(candidate) && !canApprove(candidate) && !canActivate(candidate) && !canReject(candidate) && !canRollback(candidate)" class="admin-governance-muted">只读</span></td></tr></tbody></table></div>
      </section>
