  ...
  template: `
    <section class="admin-governance-page" aria-labelledby="admin-governance-title">
      ...
      <section v-if="activeTab === 'rules'" class="admin-governance-section" aria-label="农场告警规则集">
        ...
        <div v-if="learningTotalPages > 1" class="admin-pagination">
          <button type="button" class="g-btn text sm" :disabled="learningCurrentPage <= 1" @click="learningCurrentPage--">上一页</button>
          <span class="admin-pagination-current">第 {{ learningCurrentPage }} / {{ learningTotalPages }} 页</span>
          <button type="button" class="g-btn text sm" :disabled="learningCurrentPage >= learningTotalPages" @click="learningCurrentPage++">下一页</button>
          <span>跳至 <input type="number" v-model.number="learningJumpInput" min="1" :max="learningTotalPages" class="admin-page-jump-input"> 页
            <button type="button" class="g-btn text sm" @click="learningCurrentPage = Math.max(1, Math.min(learningTotalPages, learningJumpInput || 1))">跳转</button>
          </span>
        </div>
      </section>
