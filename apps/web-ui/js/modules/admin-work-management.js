...
export const AdminWorkManagementView = {
  ...
  setup(props, { emit }) {
    ...
    const batchStatusLabel = status => statusLabel(status, '待处理');
    const availabilityLabel = status => metricStatusLabel(status, '未知');
    const riskFocusLabel = value => ({ WATER_DEFICIT: '缺水风险', HEAT_STRESS: '高温胁迫', COLD_STRESS: '低温冷害' }[String(value || '').toUpperCase()] || value || '—');
    const scheduleMethodLabel = value => ({ BY_STAGE: '按生长阶段排期', STAGE_BASED: '按生长阶段排期', MANUAL: '人工排期' }[String(value || '').toUpperCase()] || value || '—');

    return { activeTab, busy, farmId, activePlots, batches, packs, selectedPack, selectedBatch, selectedBatchId, selectedPackCode, preview, form, setTab, syncCropForPlot, createPlan, loadPlan, reviewPlan, taskLabel, stageLabel, metricLabel, value, batchStatusLabel, availabilityLabel, priorityLabel, provenanceLabel, riskFocusLabel, scheduleMethodLabel };
  },
  template: `
    <section class="admin-management-page">
      <header class="admin-section-header"><div><h1>农务任务</h1><p>任务执行、生产计划与作物模型包使用同一农场上下文。</p></div></header>
      ...
      <div v-else-if="activeTab === 'plans'" class="admin-split-layout">
        <section class="admin-panel">
          <div class="admin-panel-title"><div><span>新建批次</span><h2>生成生产计划预览</h2></div><em>推导排期</em></div>
          ...
          <p class="admin-hint">阶段区间按周期与作物模型包顺序等分，仅作为可编辑的默认排期。</p>
          ...
          <div class="admin-list-section">
            ...
            <button v-for="batch in batches" :key="batch.batchId" class="admin-list-row" :class="{selected: selectedBatchId === batch.batchId}" @click="loadPlan(batch)">
              ...
              <b>{{ batchStatusLabel(batch.status || 'ACTIVE') }}</b>
            </button>
        ...
        <section class="admin-panel">
          <div class="admin-panel-title"><div><span>计划预览</span><h2>{{ preview?.planId || '尚未选择计划' }}</h2></div><em>{{ batchStatusLabel(preview?.status) }}</em></div>
          <template v-if="preview">
            <div class="admin-plan-meta"><span>作物模型包 {{ preview.cropPackVersion || '—' }}</span><span>{{ scheduleMethodLabel(preview.scheduleMethod) }}</span><span>{{ preview.plannedCycleDays || '—' }} 天</span></div>
      ...
      <div v-else class="admin-split-layout">
        <section class="admin-panel admin-pack-list">
          <div class="admin-panel-title"><div><span>后端事实</span><h2>作物模型包版本</h2></div></div>
          ...
          <p v-if="!packs.length" class="admin-empty">后端没有返回作物模型包。</p>
        </section>
        <section class="admin-panel" v-if="selectedPack">
          ...
          <article class="admin-pack-stage" v-for="stage in selectedPack.stages" :key="stage.code">
            <div><strong>{{ stageLabel(stage.code) }}</strong><small>顺序 {{ value(stage.sequence) }} · 风险 {{ stage.riskFocus?.map(riskFocusLabel).join(' / ') || '—' }}</small></div>
            <ul><li v-for="task in stage.taskTemplates || []" :key="task.actionType">{{ taskLabel(task.actionType) }} · 每 {{ value(task.intervalDays) }} 天 · {{ priorityLabel(task.priority) }}</li><li v-if="!stage.taskTemplates?.length">任务模板：—</li></ul>
          </article>
          <h3>指标与阈值</h3>
          <div class="admin-compact-table"><div v-for="metric in selectedPack.metrics" :key="metric.code"><strong>{{ metricLabel(metric) }}</strong><span>{{ availabilityLabel(metric.availability) }}</span><span>{{ metric.range ? metric.range.min + '–' + metric.range.max + ' ' + (metric.unit || '') : '—' }}</span></div></div>
        </section>
      </div>
    </section>
  `
