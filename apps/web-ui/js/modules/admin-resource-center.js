...
export const AdminResourceCenterView = {
  ...
  setup(props, { emit }) {
    ...
    return {
      ...
      bindingLabel, deviceStatusLabel, deviceLastSeen, readableTime, healthLabel, deviceTypeLabel: adminDeviceTypeLabel, display,
      ...
      <section class="admin-panel admin-device-panel">
        ...
        <div class="admin-device-card-grid">
          <article v-for="device in visibleDevices" :key="device.deviceId" class="admin-device-card" role="button" tabindex="0"
            ...
            <footer><span>{{ device.sourceMode || '设备事实' }}</span><strong>查看详情 <app-icon name="chevron_right"></app-icon></strong></footer>
          </article>
