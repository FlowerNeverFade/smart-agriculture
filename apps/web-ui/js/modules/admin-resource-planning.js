...
export const AdminResourcePlanningView = {
  ...
  setup(props, { emit }) {
    ...
    const analyze = async () => {
      ...
      catch (caught) { notePersistenceFailure(caught); error.value = caught; toast?.(caught.message || 'AI 配水分析失败', 'error'); } finally { loading.value = false; }
    };
  ...
  template: `
    <section class="resource-ops rp-root" aria-labelledby="resource-title">
      <header class="resource-header"><div><span class="resource-eyebrow">农务任务 / 资源安排</span><h2 id="resource-title">协同配水工作台</h2><p>农户提交需求，管理员合并处方与容量约束，确认后把时段和执行结果同步回相关人员。</p></div><div class="resource-header-actions"><span class="source-badge" :class="persistenceReady ? 'is-ready' : 'is-muted'">{{ collaborationLabel }}</span><span class="resource-date">{{ profile?.businessDate || '今日计划' }}<small>{{ lastSyncedAt ? ' · ' + lastSyncedAt.toLocaleTimeString('zh-CN', {hour:'2-digit', minute:'2-digit', second:'2-digit'}) + ' 已同步' : '' }}</small></span><button class="rp-button secondary" type="button" @click="refresh" :disabled="loading">刷新</button><button class="rp-button primary" type="button" @click="analyze" :disabled="loading || collaborationReadOnly">{{ loading ? '分析中…' : '生成协同草案' }}</button></div></header>
      <div v-if="collaborationReadOnly" class="rp-error"><strong>RESOURCE_PERSISTENCE_UNAVAILABLE</strong><span>{{ collaborationLabel }}；需求、配额和计划暂时禁止写入。</span></div><div v-else-if="error" class="rp-error"><strong>{{ error.code || 'RESOURCE_PLAN_FAILED' }}</strong><span>{{ error.message || error }}</span></div><div v-if="!canManage" class="rp-empty">当前身份没有管理农场水资源的权限。</div>
      <template v-else>
