...
export const AdminResourcePlanningView = {
  ...
  setup(props, { emit }) {
    ...
    const requestStatusLabel = status => ({ SUBMITTED: '待纳入计划', IN_REVIEW: '方案编制中', PENDING_ACK: '待农户确认', ACKNOWLEDGED: '农户已确认', CONFLICT_REPORTED: '农户反馈冲突', COMPLETED: '已完成', CANCELLED: '已撤回' }[String(status || '').toUpperCase()] || status || '待处理');
  ...
  template: `
    <section class="resource-ops rp-root" aria-labelledby="resource-title">
      <header class="resource-header"><div><span class="resource-eyebrow">农务任务 / 资源安排</span><h2 id="resource-title">协同配水工作台</h2><p>农户提交需求，管理员合并处方与容量约束，确认后把时段和执行结果同步回相关人员。</p></div><div class="resource-header-actions"><span class="source-badge" :class="state.sessionMode === 'live' ? 'is-ready' : 'is-muted'">{{ state.sessionMode === 'live' ? '后端协同' : '演示数据' }}</span><span class="resource-date">{{ profile?.businessDate || '今日计划' }}<small>{{ lastSyncedAt ? ' · ' + lastSyncedAt.toLocaleTimeString('zh-CN', {hour:'2-digit', minute:'2-digit', second:'2-digit'}) + ' 已同步' : '' }}</small></span><button class="rp-button secondary" type="button" @click="refresh" :disabled="loading">刷新</button><button class="rp-button primary" type="button" @click="analyze" :disabled="loading">{{ loading ? '分析中…' : '生成协同草案' }}</button></div></header>
