...
浏览器访问：`http://127.0.0.1:3000/login.html`。不要使用 `python -m http.server` 启动登录页；静态服务器不支持认证接口的 `POST`，会返回 `501 Unsupported method`。

### 2. 模式说明（后端在线 / 纯离线双模）
- **离线演示模式**：未启动 Java 后端时，必须先点击“演示身份”并明确选择一个角色；字段保持未修改即可直接建立本地演示会话，不发送登录 `POST`。
- **正式登录模式**：手工输入账号、密码或身份时始终请求真实认证接口；后端未启动会显示启动提示，不会自动降级为演示身份。
- **后端联调模式**：启动 Spring Boot 后端服务（默认端口 8080）后，包括演示账号在内的登录都通过真实认证接口取得 JWT；页面顶栏随后显示后端在线状态。
